...
namespace Recetario
{
    internal class Recetario : RecetarioBaseVisitor<object>
    {
        ...
        int IDElaboracion = 1;
        int IDIngrediente = 1;
        string InsertElaboracion = "";
        string InsertIngredientes = "";
        ...
        public override object VisitElaboracion([NotNull] RecetarioParser.ElaboracionContext context)
        {
            ...
            {
                InsertElaboracion += $"INSERT INTO cooking_steps (id, recipe_id, step_number, description) VALUES ({IDElaboracion}, {IDReceta}, {i}, '{Visit(item)}');\n";
                i++;
                IDElaboracion++;
            }
            return InsertElaboracion;
        }
        ...
        public override object VisitIngredientes([NotNull] RecetarioParser.IngredientesContext context)
        {
            ...
            foreach (var item in context.ing_item())
            {
                ...
                if (ingItem.Length == 3)
                {
                    InsertIngredientes += $"INSERT INTO ingredients (id, recipe_id, name, quantity, unit_id) VALUES ({IDIngrediente}, {IDReceta}, '{ingItem[0]}', {ingItem[1]}, {ingItem[2]});\n";
                }
                else if (ingItem.Length == 2)
                {
                    InsertIngredientes += $"INSERT INTO ingredients (id, recipe_id, name, quantity, unit_id) VALUES ({IDIngrediente}, {IDReceta}, '{ingItem[0]}', {ingItem[1]});\n";
                }
                IDIngrediente++;
            }
            return InsertIngredientes;
        }
        ...
        public override object VisitReceta([NotNull] RecetarioParser.RecetaContext context)
        {
            ...
            string insertValues = $"VALUES ({IDReceta}, '{nombre}', {porciones},";
            ...
            string inserts = recipes + InsertIngredientes + InsertElaboracion;
            output.Add(inserts);
            return inserts;
        }
